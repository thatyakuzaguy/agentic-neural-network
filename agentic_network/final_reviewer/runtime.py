"""Final Reviewer Agent runtime, parser, validation, and model adapter."""
...
FINAL_REVIEW_OUTPUT_FILE = "08_final_review.md"
...
FINAL_REVIEW_LIST_KEYS = tuple(
    key
    for title, key in SECTION_KEYS.items()
    if title not in {"FINAL DECISION", "CONFIDENCE"}
)
...
FORBIDDEN_FIX_OR_COMMAND_PATTERNS = {
    ...
    "forbidden_fix_instructions": re.compile(
        ...
        r"change\s+the\s+code\s+to|add\s+the\s+following\s+code|implementation\s+plan:)\b"
    ),
}


@dataclass(frozen=True)
class FinalReviewerResult:
    """Structured Final Reviewer output returned to the pipeline and CLI."""
    ...
    def to_stage_output(self) -> str:
        """Return the final review artifact saved as the pipeline final stage."""

        return self.final_review_output


class FinalReviewerRuntimeModel(BaseModelClient):
    """BaseModelClient adapter for artifact-only final approval review."""

    def __init__(self, config: PipelineConfig, *, mode: str = "fast") -> None:
        self.config = config
        self.mode = _normalize_mode(mode)
        self.last_result: FinalReviewerResult | None = None
        self._model: BaseModelClient | None = None

    @property
    def backend_name(self) -> str:
        return f"final_{self.mode}"
    ...
    def generate_text(self, prompt: str) -> str:
        result = run_final_reviewer_agent(
            ...
            fix_plan=_extract_context_section(prompt, "FIXES"),
            mode=self.mode,
    ...
    def diagnostics(self) -> dict[str, object]:
        diagnostics: dict[str, object] = {
            ...
            "loaded_backend_type": "final_reviewer_runtime",
            "final_reviewer_mode": self.mode,
        }
    ...
    def _generate_with_model(self, *, prompt: str) -> str:
        if self._model is None:
            self._model = _model_for_final_mode(self.config, self.mode)
        return self._model.generate_text(prompt)


def run_final_reviewer_agent(
    *,
    user_request: str,
    ...
    """Run the Final Reviewer Agent and return parsed output plus validation details."""

    normalized_mode = _normalize_mode(mode)
    prompt = build_final_reviewer_prompt(
    ...
    final_output, parsed_sections, warnings, validation_errors = _clean_parse_and_validate(
    ...
    if validation_errors:
        ...
        fallback_used = True
        final_output = _safe_final_review()
    ...
    if output_artifact_path is not None:
        ...
        output_artifact_path.write_text(final_output.rstrip() + "\n", encoding="utf-8")
        written_path = str(output_artifact_path)
    return FinalReviewerResult(
...
def build_final_reviewer_prompt(
    *,
    ...
    """Build the strict Final Reviewer prompt."""
    ...
    return "\n\n".join(
        [
            ...
            "FIXER AGENT PLAN",
            fix_plan.strip(),
            "FINAL REVIEWER AGENT OUTPUT",
            "",
        ]
    )


def parse_final_reviewer_sections(response: str) -> dict[str, list[str] | str]:
    """Split a Final Reviewer response into normalized named sections."""
    ...
    for raw_line in response.splitlines():
        ...
        if heading_match:
            ...
            sections[key] = "" if key in {"final_decision", "confidence"} else []
            continue
        if current_heading is None:
            continue
        key = SECTION_KEYS[current_heading]
        if key in {"final_decision", "confidence"}:
...
def validate_final_reviewer_response(
    *,
    final_review_output: str,
    parsed_sections: dict[str, list[str] | str],
    raw_response: str | None = None,
) -> tuple[list[str], list[str]]:
    """Validate the Final Reviewer output contract."""

    warnings: list[str] = []
    errors: list[str] = []
    raw_text = raw_response if raw_response is not None else final_review_output
    section_counts = _section_counts(final_review_output)
    for title, key in SECTION_KEYS.items():
    ...
    final_decision = str(parsed_sections.get("final_decision", "")).strip()
    if not final_decision:
        errors.append("final_decision_missing")
    elif final_decision not in FINAL_DECISIONS:
        errors.append("final_decision_invalid")
    ...
    validation_texts = (raw_text, final_review_output)
    if re.search(r"</?think\b", final_review_output, re.IGNORECASE):
        errors.append("think_tags_present")
    if re.search(r"(?m)^\s*#{1,6}\s+", final_review_output):
...
def clean_final_reviewer_response(response: str) -> str:
    """Clean model chatter into the artifact-only final review contract."""
    ...
    for raw_line in cleaned.splitlines():
        ...
        if heading_match:
            if current_heading == "FINAL DECISION" and (
                _last_nonblank_line(output_lines) not in FINAL_DECISIONS
            ):
            ...
            skip_scalar = current_heading in {"FINAL DECISION", "CONFIDENCE"}
            continue
        if skip_scalar and current_heading == "FINAL DECISION":
            output_lines.append(_normalize_final_decision(stripped))
            skip_scalar = False
            continue
...
def _clean_parse_and_validate(
    *,
    ...
    final_output = clean_final_reviewer_response(raw_response)
    parsed_sections = parse_final_reviewer_sections(final_output)
    ...
    if raw_had_think_tags and not _has_think_tags(final_output):
        warnings.append("think_tags_removed")
    return final_output, parsed_sections, warnings, validation_errors
    ...
    *,
    ...
    return "\n\n".join(
        [
            ...
            "Do not output commands, code, patches, diffs, fixes, or implementation plans.",
            "Use FINAL DECISION as either Approved or Rejected.",
            "INVALID OUTPUT",
            invalid_output,
            "REGENERATED FINAL REVIEWER AGENT OUTPUT",
            "",
        ]
    )


def _safe_final_review() -> str:
    return "\n".join(
        [
            "FINAL ASSESSMENT",
            "* Final review could not be completed safely.",
            "",
            "REQUIREMENTS STATUS",
...
def _model_for_final_mode(config: PipelineConfig, mode: str) -> BaseModelClient:
    normalized_mode = _normalize_mode(mode)
    if normalized_mode == "deep":
        deep_config = _final_deep_config(config)
        if deep_config.deepseek_gguf_path is not None:
            return DeepSeekGGUFModel(deep_config)
        return DeepSeekUnslothModel(deep_config)
    return Qwen3Model(_final_fast_config(config))


def _final_fast_config(config: PipelineConfig) -> PipelineConfig:
    fast_model = config.final_reviewer_fast_model
...
def _final_deep_config(config: PipelineConfig) -> PipelineConfig:
    deep_model = config.final_reviewer_deep_model
...
def _normalize_final_decision(value: str) -> str:
    ...
    if normalized in {"approved", "approve", "accepted", "pass", "status approved"}:
        return "Approved"
    if normalized in {
        "rejected",
        "reject",
        ...
        return "Rejected"
    return value.strip() or "Rejected"
...
def _unsupported_real_generator(*, prompt: str) -> str:
    raise RuntimeError(
        "No Final Reviewer Agent response generator was provided. Use "
        "FinalReviewerRuntimeModel for real pipeline execution or pass a fake "
        "response_generator for smoke tests."
    )
